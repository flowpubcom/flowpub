...
> `flowpub.app` se queda registrado en Namecheap** y solo se apunta por DNS.
...
3. **Environment Variables** (antes del primer deploy) — mismos nombres que
   ...
   | `NEXT_PUBLIC_SITE_URL` | `https://flowpub.app` |
   | `SUPABASE_SERVICE_ROLE_KEY` · `TURNSTILE_*` · `RESEND_API_KEY` | cuando toquen sus fases |

4. **Deploy** → te da `flowpub-….vercel.app` para probar.

## 3) Apuntar flowpub.app (DNS en Namecheap)

1. En Vercel: **Project → Settings → Domains** → agrega `flowpub.app` y
   `www.flowpub.app`. Vercel te muestra los registros exactos.
2. En Namecheap: **Domain List → flowpub.app → Advanced DNS** → borra los
   ...
   al conectar el dominio en Vercel, porque `flowpub.app` (sin www) es la URL
   canónica en todo el sitio (`NEXT_PUBLIC_SITE_URL`, sitemap, OpenGraph). Si
   más adelante quieres que `www.flowpub.app` también funcione, agrégalo en
   Vercel como dominio con "Redirect to Another Domain" → `flowpub.app`.
...
- **Auth → URL Configuration**: Site URL = `https://flowpub.app`; en Redirect
  URLs agrega `https://flowpub.app/**` (conserva `http://localhost:3000/**`
  para dev).
- **Google Cloud → OAuth client**: agrega `https://flowpub.app` a *Authorized
